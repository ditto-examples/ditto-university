        ...
        try {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                try {
                    ...
                    this@DittoManager.ditto?.store?.execute("ALTER SYSTEM SET DQL_STRICT_MODE = false")
                    // TODO: setup sync scope for task collection to only sync with nearby devices
                    
                } catch (e: DittoError) {
                    // Handle specific Ditto errors related to the execute call if necessary
                    Log.e(TAG, "Failed to set DQL_STRICT_MODE: ${e.message}", e)
                    errorService.showError("Failed to configure Ditto store: ${e.message}")
                } catch (e: Exception) {
                    // Catch any other unexpected errors
                    Log.e(TAG, "Unexpected error during Ditto store configuration: ${e.message}", e)
                    errorService.showError("Unexpected error configuring Ditto: ${e.message}")
                }
            }
        } catch (e: DittoError) {
